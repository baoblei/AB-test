...
- 多维度评测：
  - `T2I`：美学、合理性、一致性
  - `TI2I`：美学、合理性、一致性、保真度
  - `T2V`：整体、文本一致性、结构合理性、运动合理性、动态度、物理规律与常识、画面细节与美感
...
- `/login`：登录页
- `/dashboard`：统计看板
- `/help`：用户侧平台使用说明，重点解释看板操作和统计指标计算口径
- `/profile`：个人中心
- `/admin`：管理后台

### 用户侧使用说明与统计口径

看板顶部提供“使用帮助”按钮，点击后进入 `/help`。该页面面向评测员、管理员和统计结果查看者，不包含安装、部署或代码说明。

阅读看板前，必须先区分以下三个数量：

| 名称 | 当前计算口径 |
| --- | --- |
| 评测记录数 | 一次未跳过的提交计 1 条；同一文件由 3 人评价会产生 3 条记录 |
| 去重样例并集数 | 按“任务类型 + 规范化模型对 + 场景 + 文件名”去重；用于“并集冲突比例”的分母 |
| 多人共同评价样例数（交集数） | 同一去重样例在当前筛选范围内至少被两位不同评测员有效评价；用于“交集冲突比例”的分母 |

模型对标题、场景表和评测员统计中的“样本数”当前表示**评测记录数**，不是去重文件数。各维度只统计该维度值不为空的记录。

#### A/B、平局与百分比

- A 胜、B 胜、一样差、一样好均按评测记录计数。
- 同一文件由多人评价时，每人的选择分别计入。
- `某类百分比 = 该类评测记录数 ÷ 当前维度四类评测记录总数 × 100%`。
- 一样差和一样好分别展示，但在平局总数和压制比中合并使用。

#### 压制比

```text
A 压制 =（A 胜 + 一样差 + 一样好）÷（B 胜 + 一样差 + 一样好）
B 压制 =（B 胜 + 一样差 + 一样好）÷（A 胜 + 一样差 + 一样好）
```

分母为 0 且分子大于 0 时显示 `∞`；两者都为 0 时显示 `-`。压制比不是胜率，因为平局同时进入两个方向。

#### 冲突宽容度

同一去重样例、同一维度内，系统按评测员身份去重，只统计 A 更好和 B 更好的票；一样差、一样好不参与冲突票数。

```text
少数方占比 = min(A 票人数, B 票人数) ÷ (A 票人数 + B 票人数)
```

只有两侧都有人选择，且少数方占比**严格大于**宽容度时才判定为冲突；等于宽容度不算冲突。因此：

- `0%` 最严格，只要不同评测员存在相反票就冲突。
- `20%` 时，4 人选 A、1 人选 B 的少数方占比正好为 20%，不冲突。
- `50%` 时，即使一半选 A、一半选 B，也不冲突。

#### 并集冲突比例与交集冲突比例

两个比例使用同一个分子：当前维度被判为冲突的去重样例数；区别只在分母。

```text
并集冲突比例 = 冲突样例数 ÷ 去重样例并集数
交集冲突比例 = 冲突样例数 ÷ 多人共同评价样例数
```

- 去重样例并集数：当前维度至少有一条有效评分的全部去重样例，包含只被一人评价的样例。
- 多人共同评价样例数（交集数）：当前维度至少被两位不同评测员有效评价的去重样例。选择具体评测员后，只在所选人员范围内重新计算。
- 多于两位评测员时，只要样例被当前范围内至少两位不同评测员评价，就进入交集分母；这保证所有可能发生冲突的样例都包含在交集分母中。
- 平局评分可以使样例进入并集或交集分母，但平局票不参与冲突判定。
- 分母为 0 时比例显示为 0.0%。

例如两位评测员各评 50 张，只有 9 张重叠：

```text
去重样例并集数 = 50 + 50 - 9 = 91
```

此时并集分母为 91，交集分母为 9。假设其中 1 张被判为冲突：
...
并集比例反映冲突占全部已覆盖样例的范围，交集比例反映多人共同评价样例中的意见分歧强度，判断结果可靠性时应结合两者。

勾选“统计时去除冲突项”后，只从发生冲突的那个维度移除该样例的全部评测记录，再计算胜负、平局、压制比和排行榜；原始冲突分子以及并集、交集分母仍保留。明细或评测员统计选择具体评测员后，冲突和两个分母都会在当前人员范围内重新计算。
...
- 一条记录选择多个标签，坏例记录数仍只加 1。
- 高频坏例按具体标签出现次数排序。
- 同一记录选择同一类别下多个标签时，类别记录数只加 1，各具体标签分别加 1。
...
- 当前维度每有一条有效评测记录，参与该记录的两个模型对战数各加 1。
- 选择某模型胜出时，该模型胜场数加 1。
- 一样差、一样好增加双方对战数，但不增加胜场数。
- 场景、冲突宽容度和“去除冲突项”会影响排行榜结果。

#### 个人与管理统计

- 个人总评测数：本人未跳过的评测记录数。
- 个人场景分布：本人未跳过记录按场景计数。
- 个人平均耗时：本人所有存在耗时值的结果记录平均秒数；当前实现包含跳过记录。
- 管理后台用户数：账号总数，包含停用账号。
- 管理后台总评测数、今日评测：未跳过记录总数及北京时间当天记录数。
- 管理后台模型数：A 列去重模型数加 B 列去重模型数；同一模型若出现在两列，可能重复计数。
