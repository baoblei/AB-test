...
| 去重样例并集数 | 按“任务类型 + 规范化模型对 + 场景 + 文件名”去重；用于“冲突样例 X/Y”的分母 |
| 多人共同评价数 | 同一去重样例至少被两位不同评测员评价；当前看板未单独展示该值 |
...
- 分子 `X`：当前维度被判为冲突的去重样例数。
- 分母 `Y`：当前维度至少有一条有效评分的去重样例并集数。
- `Y` 不是评测记录总数、不是测评集文件总数，也不是多人共同评价数。
- 平局评分可以使样例进入 `Y`，但平局票不参与冲突判定。
...
此时冲突分母显示 91。只有一人评价的样例不可能产生人员冲突，所以 `0/91` 不表示 91 张都经过多人一致性验证。

勾选“统计时去除冲突项”后，只从发生冲突的那个维度移除该样例的全部评测记录，再计算胜负、平局、压制比和排行榜；原始冲突分子、分母仍保留。明细或评测员统计选择具体评测员后，冲突会在当前人员范围内重新计算。
