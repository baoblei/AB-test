...
- XLSX 始终包含 `Overall`；其余 Sheet 按场景生成，维度选择控制场景 Sheet 中横向展示的美学、合理性、一致性和 TI2I 保真度分组。
...
```python
def test_t2i_workbook_has_overall_and_scene_sheets(self):
...
def test_scene_detail_contains_filename_prompt_worker_and_grouped_dimensions(self):
    ...
    self.assertIn("美学", [cell.value for cell in sheet[1]])
...
def test_ti2i_workbook_has_fidelity_and_reference_columns(self):
    sheet = build_workbook(self.ti2i_request, self.ti2i_rows)["portrait"]
    headers = [cell.value for cell in sheet[2]]
...
按场景创建 Sheet，并清理 Excel 非法字符、31 字符上限和清理后的重名。Base columns are task type, model A/B, scene, filename, Prompt, worker, eval mode, Beijing timestamp, A/B image paths and statuses. Conditionally append duration and bad-case columns. TI2I appends ref path/status.

表头使用两行：第一行按样本信息、各维度、图片信息和坏例信息横向合并；第二行放具体字段。每个已选维度按任务配置顺序占三列，分别表示规范模型 A 胜、平局、规范模型 B 胜，命中列写入数值 `1`。场景行取各维度筛选结果的并集，未命中当前筛选的维度组保持为空。
